...
#'   `$isNC` (true labels) and `$tool_combinations` (list of prediction
...
bestToolComb <- function(sumCombToolsList, combinationsToAnalyze = NULL, digits = 4) {
  ...
  availableCombinations <- names(sumCombToolsList$tool_combinations)
  referenceLabels <- sumCombToolsList$isNC

  if (is.null(availableCombinations) || length(availableCombinations) == 0) {
    warning("No tool combination names found in sumCombToolsList$tool_combinations.", call. = FALSE)
    return(NULL)
  }
  ...
  for (combName in selectedCombinations) {
    predictionVector <- sumCombToolsList$tool_combinations[[combName]]
...
validateBestToolCombInput <- function(sumCombToolsList) {
  requiredElements <- c("isNC", "tool_combinations")
  if (!validateInputList(sumCombToolsList, requiredElements)) {
    return(FALSE)
  }
  if (!is.list(sumCombToolsList$tool_combinations) || length(sumCombToolsList$tool_combinations) == 0) {
    warning("Input list '$tool_combinations' must be a non-empty list.", call. = FALSE)
    return(FALSE)
  }
