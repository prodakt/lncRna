...
#'   `$isNC` (numeric vector of true labels: 1=nc, 0=cds) and `$at_least_n`
...
bestToolAtLeast <- function(sumAtLeastList, digits = 4) {
  ...
  # --- 1. Input Validation ---
  requiredElements <- c("isNC", "at_least_n")
  if (!validateInputList(sumAtLeastList, requiredElements)) {
    return(NULL)
  }

  if (!is.list(sumAtLeastList$at_least_n) || length(sumAtLeastList$at_least_n) == 0) {
    warning("Input list '$at_least_n' must be a non-empty list.", call. = FALSE)
    return(NULL)
  }
  ...
  atLeastNPredictions <- sumAtLeastList$at_least_n
