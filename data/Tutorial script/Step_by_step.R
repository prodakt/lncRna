...
library(gprofiler2)
library(tidyr)
library(fsmb) # ERROR library(fmsb)
library(patchwork)
library(ggplot2)
...
# We need to read two GTF (Gene Transfer Format) files: # WARNING "You need to ..."
...
# This is a basic filtering step to remove transcripts with very low or no detectable expression. # ERROR!!! without "or no detectable..."
...
potential_lncRNA_transcripts <- tab1[tab1$transcript_id %!in% known_pcRNA,] # ERROR!!!! is not created earlier!!!!
...
{ # ERROR !!! in test.train.cds and test.train.nc functions !!!!!
...
cds_training_test_sets <- test.train.cds(cds.fa = cds_reference_sequences, percent_train = 0.6) # ERROR!!! inside function cds is needed!!!
set.seed(12345) # Set seed for reproducible training/test set splitting
nc_training_test_sets <- test.train.nc(nc.fa = nc_reference_sequences, percent_train = 0.6) # ERROR!!! inside function nc is needed!!!
...
best_tool_combination_analysis <- BestTool.comb(BestPat = combined_tool_performance_all_combinations, selectComb = selected_combinations) # ERROR!!! selectedCombinations ->  selected_combinations
...
radar_plot_cm(cm_list = all_high_precision, methods = c("CPC2+CPAT", "PLEK+CPAT", "CPAT+CNCI")) # WARNING !!!!
...
radar_plot_cm(cm_list = all_high_precision, metrics = c("Sensitivity", "Recall", "Precision", "Accuracy")) # WARNING !!!! how to list all available metrics?
...
radar_plot_cm(cm_list = all_high_precision, layout = "multiple", display_area = TRUE, display_fill = FALSE, save_data = TRUE, file_name = "radar_plot_data_banana") # WARNING!!!! should it be "radar_plot_data_banana.csv"?
...
# ERROR !!! there is no "all_cms" object
...
clock_plot_cm(cm_list = all_high_precision, methods = c("CPC2+CPAT", "PLEK+CPAT", "CPAT+CNCI")) # WARNING !!!
...
#cis_enrichment_results <- gost(query = cis_interaction_table$partnerRNA_gene, # WARNING!!! partnerRNA_gene/partnerRNA_transcript see CisInter() inputs !!!!!
...
#LION_enrichment_results <- gost(query = c(LION_proteins_method1, LION_proteins_method2, LION_proteins_method3), # WARNING !!!!
...
Trans_interactions_processed <- process_interactions(gprof = trans_gProfiler_results_table, interaction_table = trans_interaction_table, type = "trans")  # ERROR!!! trans_enrichment_results -> 
Cis_interactions_processed <- process_interactions(gprof = cis_gProfiler_results_table, interaction_table = cis_interaction_table, type = "cis") # ERROR !!! cis_enrichment_results -> 
LncTar_interactions_processed <- process_interactions(gprof = LncTar_gProfiler_results_table, interaction_table = LncTar_interaction_data, type = "LncTar", lncRNA_col = "Query", target_col = "Target") # ERROR !!! LncTar_enrichment_results -> 
LION_interactions_processed <- process_interactions(gprof = LION_gProfiler_results_table, interaction_table = LION_interaction_data, type = "LION", lncRNA_col = "RNA_Name", target_col = "Pro_Name") # ERROR !!! LION_enrichment_results -> 
...
plot_by_type(data = Cis_interactions_processed, type = "cis", label = TRUE) # ERROR !!! there is no "selected_type" object

# **Example 3: 'plot_by_type()' for combined 'cis' and 'LncTar' interaction types, with labels.**
plot_by_type(data = combined_interactions_table, type = c("cis", "LncTar"), label = TRUE) # ERROR !!! there is no "selected_type" object
