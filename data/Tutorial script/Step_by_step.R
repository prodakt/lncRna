...
# This is a basic filtering step to remove transcripts with very low or no detectable expression. # ERROR!!! without "or no detectable..." jpj
...
potential_lncRNA_transcripts <- tab1[tab1$transcript_id %!in% known_pcRNA,] # ERROR!!!! is not created earlier!!!! jpj
...
{ # WARNING !!!! jpj
  ###  ================ 4.6: Cleaning the environment ================
  # removing from the environment unnecessary big data
  rm()
...
# The 'coding_potential_list' (assigned to 'tbl2' in the original script) now contains the combined
# coding potential prediction results from all specified tools.
head(coding_potential_list) # Inspect the first few rows of the combined table.

# Assume 'coding_potential_list' is already generated by CodPot2tbl()
# For example:
# coding_potential_list <- CodPot2tbl(CPC2_outfile = "cpc2.txt", ...)

# Assume 'my_custom_tool_noncoding_ids' contains IDs predicted as non-coding by your tool
# For demonstration, let's create a sample:
set.seed(123) # for reproducibility
if (length(coding_potential_list$seqIDs) > 0) {
  my_custom_tool_noncoding_ids <- sample(coding_potential_list$seqIDs,
                                         size = floor(length(coding_potential_list$seqIDs) * 0.3)) # 30% as non-coding
} else {
  my_custom_tool_noncoding_ids <- character(0) # Handle empty case
}


# 1. Prepare the prediction vector for your custom tool
# Initialize a vector of 0s with the same length as seqIDs
num_sequences <- length(coding_potential_list$seqIDs)
my_custom_tool_predictions <- numeric(num_sequences) # Default to 0 (coding)

# Find the indices of your non-coding IDs within the main seqIDs list
# The %in% operator is efficient for this
indices_of_noncoding <- which(coding_potential_list$seqIDs %in% my_custom_tool_noncoding_ids)

# Set predictions to 1 for these non-coding sequences
if (length(indices_of_noncoding) > 0) {
  my_custom_tool_predictions[indices_of_noncoding] <- 1
}

# 2. Add this prediction vector to the 'tools' sublist
# Choose a name for your tool, e.g., "MyCustomTool"
coding_potential_list$tools$MyCustomTool <- my_custom_tool_predictions

# Verify (optional):
# print(names(coding_potential_list$tools))
# if (num_sequences > 0) {
#   print(head(data.frame(ID = coding_potential_list$seqIDs, MyTool = coding_potential_list$tools$MyCustomTool)))
# }

# Now, 'coding_potential_list' includes predictions from "MyCustomTool".
# This updated list can be passed to subsequent analysis functions.
...
# This generates a Venn diagram showing the overlap in non-coding predictions across all tools in 'coding_potential_list'.
venn.CodPot(CodPot = coding_potential_list)
...
# It takes the combined coding potential list ('coding_potential_list'), and the test sets
# for non-coding RNAs ('nc_tt$nc.test') and coding RNAs ('cds_tt$cds.test') as input.

individual_tool_performance <- SumSingleTools(CodPot_list = coding_potential_list, nc_test = nc_training_test_sets$nc.test, cds_test = cds_training_test_sets$cds.test) # ERROR !!! Poprawione nazwy zmiennych
...
selected_tools <- c("CPC2", "PLEK", "FEELnc", "CPAT", "CNCI", "LncFinder") # vector of tools to analyze
...
# From the comprehensive 'combined_tool_performance_all_combinations' list, we can select
# specific tool combinations that are of interest (e.g., based on Venn diagram visualization or
# initial performance observations) by console or created previously vector.
# The 'BestTool.comb()' function performs error analysis for these selected combinations.


best_tool_combination_analysis <- BestTool.comb(SumCombTools_list = combined_tool_performance_all_combinations)
...
all_confusion_matrices <- calculate_cm(SumCombTools_list = combined_tool_performance_all_combinations, print_metric_threshold_methods = TRUE, threshold = 0.9, return_only_high_methods = TRUE, metric_to_extract = "Precision")
all_confusion_matrices <- lncRna::calculate_cm(bp_cmb_data = best_tool_combination_analysis, best_pat3_data = combined_tool_performance_all_combinations_check, print_metric_threshold_methods = TRUE, threshold = 0.9, return_only_high_methods = TRUE, metric_to_extract = "Precision")

# **Calculate confusion matrices and print metric thresholds for methods (default metric: Accuracy):**
all_confusion_matrices_thresholds <- lncRna::calculate_cm(bp_cmb_data = best_tool_combination_analysis, best_pat3_data = combined_tool_performance_all_combinations_check, print_metric_threshold_methods = TRUE)

# **Calculate confusion matrices with a specific threshold value (e.g., threshold = 0.9 for Accuracy):**
all_confusion_matrices_threshold_0.9 <- lncRna::calculate_cm(bp_cmb_data = best_tool_combination_analysis, best_pat3_data = combined_tool_performance_all_combinations_check, print_metric_threshold_methods = TRUE, threshold = 0.9, return_only_high_methods = TRUE)

# **Calculate confusion matrices for a specific metric (e.g., "Precision") and return only high-performing methods:**
high_precision_confusion_matrices <- lncRna::calculate_cm(bp_cmb_data = best_tool_combination_analysis, best_pat3_data = combined_tool_performance_all_combinations_check, print_metric_threshold_methods = TRUE, metric_to_extract = "Precision", return_only_high_methods = TRUE)

# **Alternatively, return only the list of high-performing methods (meeting the threshold for Precision):**
all_high_precision <- lncRna::calculate_cm(bp_cmb_data = best_tool_combination_analysis, best_pat3_data = combined_tool_performance_all_combinations_check, print_metric_threshold_methods = TRUE, return_only_high_methods = TRUE)
...
radar_plot_cm(cm_list = all_high_precision, layout = "multiple", display_area = TRUE, display_fill = FALSE, save_data = TRUE, file_name = "radar_plot_data_banana") # WARNING!!!! should it be "radar_plot_data_banana.csv"? jpj
...
# predicted_lncRNA_candidates <- coding_potential_list[coding_potential_list$seqIDs %in% potential_lncRNA_transcript_ids,] # Use 'potential_lncRNA_transcript_ids' if you used pre-filtered list for prediction
predicted_lncRNA_candidates <- coding_potential_list # If you predicted on the whole transcriptome, use the full coding potential table
...
#cis_enrichment_results <- gost(query = cis_interaction_table$partnerRNA_gene, # WARNING!!! partnerRNA_gene/partnerRNA_transcript see CisInter() inputs !!!!! jpj
...
plot_by_type(data = combined_interactions_table, type = c("cis", "LncTar"), label = TRUE) # ERROR !!! there is no "selected_type" object jpj
