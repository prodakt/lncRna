...
install_github("prodakt/lncRna@test", force = T)
#options(timeout=400) 
...
##########################################################################
setwd("D://NGS/lncRna/lncRna-test/")
...
transcriptome <- read.fasta("data/merged_transtriptome.fa", seqtype = "DNA", as.string = F, set.attributes = T)
head(transcriptome)
head(names(transcriptome))
tail(names(transcriptome))

seqs2predict <- transcriptome
# OR
seqs2predict <- read.fasta(".../merged_transtriptome.fa", seqtype = "DNA", as.string = T, set.attributes = F)
seqs2predict <- read.fasta("data/merged_transtriptome.fa", seqtype = "DNA", as.string = T, set.attributes = F)
...
all_high_precision <- calculate_cm(bp_cmb_data = BP.cmb, best_pat3_data = BestPat3, print_metric_threshold_methods = T, 
                                   metric_to_extract = "Precision", return_only_high_methods = T)
...
radar_plot_cm(cm_list = all_high_precision, display_fill = F)
radar_plot_cm(cm_list = all_high_precision, display_fill = F)
...
# an example of running direct interactions:
# LncTar for LncRNA - Target mRNA interactions ------
...
# download and unpack LncTar tool
# in terminal !!!
# cd path/to/LncTar
# export PERL5LIB=/path/to/LncTar:$PERL5LIB

# /////////////////////////////////////////////////////////////////////////////////////////////
# LION for RNA-Protein interacion ------
...
head(refGTF)
#DEGs_prot <- refGTF[refGTF$type %in% "transcript",]
DEGs_prot <- refGTF
...
# Sprawdzenie, czy oba pliki zawierają sekwencje
...
# Tworzymy kombinacje RNA i Protein
combinations <- expand.grid(RNA = names(rna_seqs), Protein = names(prot_seqs))
nrow(combinations)

# Sprawdzamy, czy liczba kombinacji jest parzysta
if (nrow(combinations) %% 2 == 1) {
  combinations <- rbind(combinations, combinations[nrow(combinations), ])  # Powtarzamy ostatnią kombinację
}

# Liczba zestawów to liczba wierszy podzielona przez 2
total_combinations <- nrow(combinations) / 2
combination_counter <- 0  # Licznik aktualnej kombinacji

# Inicjalizacja pustej tabeli wynikowej
LION_res <- NULL

# Iteracja po parach (co dwa wiersze)
for (i in seq(1, nrow(combinations), by = 2)) {
  
  # Selekcja dwóch pierwszych kombinacji (RNA vs Protein)
  ...
  # Tworzymy zmienne rna_subset i prot_subset (po dwie sekwencje)
  rna_subset <- list(
    rna_seqs[[rna_name_1]],
    rna_seqs[[rna_name_2]]
  )
  ...
  # Aktualizacja zbiorczego wyniku
  if (is.null(LION_res)) {
  ...
  # Zapisanie wyników co iterację i nadpisanie pliku
